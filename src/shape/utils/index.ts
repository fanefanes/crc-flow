...
export function optimizeMultilineText(
  text: string, font: string, maxRows: number, maxWidth: number,
) {
  ...
  if (multilineText.length > maxRows) {
    ...
    let mtempText = '';
    const mtempTextWidth = 0;

    multilineText[maxRows - 1].split('').forEach((char) => {
      const { width } = canvasContext.measureText(char);

      if (mtempTextWidth + width > maxWidth - ellipsisWidth) {
        return;
      }
    ...
    multilineText = multilineText.slice(0, maxRows - 1).concat(`${mtempText}${ellipsis}`);
  }
