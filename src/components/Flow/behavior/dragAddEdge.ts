...
import {
  ItemType, ItemState, GraphType, AnchorPointState, GraphCustomEvent,
} from '@/common/constants';
import {
  Node, Edge, Behavior, GraphEvent, EdgeModel, AnchorPoint,
...
interface DefaultConfig {
  ...
  edgeType: string;
  /** 获取来源节点锚点状态 */
  getAnchorPointStateOfSourceNode(
    sourceNode: Node, sourceAnchorPoint: AnchorPoint
  ): AnchorPointState;
  /** 获取目标节点锚点状态 */
...
const dragAddEdgeBehavior: DragAddEdgeBehavior & ThisType<DragAddEdgeBehavior & DefaultConfig> = {
  ...
  handleNodeMouseEnter(e) {
    ...
    const sourceAnchorPointsState: any[] = [];
  ...
  handleNodeMouseDown(e) {
    if ((this.shouldBegin && !this.shouldBegin(e)) || !this.shouldAddDelegateEdge(e)) {
      return;
    }
    ...
    graph.getNodes().forEach((targetNode) => {
      ...
      const targetAnchorPointsState: any[] = [];

      targetAnchorPoints.forEach((targetAnchorPoint) => {
        targetAnchorPointsState.push(
          getAnchorPointStateOfTargetNode(
            sourceNode, sourceAnchorPoint, targetNode, targetAnchorPoint,
          ),
        );
      });
